...
def _looks_like_variable(content: str) -> bool:
    """Check if content looks like a Python variable name (not prose).

    Returns True if content is a single word that is a valid Python identifier,
    meaning it could potentially be a variable name that needs resolution.
    """
    # Single word, valid Python identifier, no spaces or special chars
    return bool(re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", content))
...
def find_final_answer(text: str, environment: "BaseEnv | None" = None) -> str | None:
    ...
    # Use string parsing to handle parentheses properly (e.g., "FINAL(Answer with (parentheses) inside)")
    for line in text.split("\n"):
        stripped = line.strip()
        if stripped.startswith("FINAL(") and stripped.endswith(")"):
            # Extract content between "FINAL(" and the final ")"
            content = stripped[6:-1].strip()  # Remove "FINAL(" (6 chars) and ")" (1 char)

            # Smart variable detection: if content looks like a variable name,
            # try to resolve it using FINAL_VAR mechanism (handles LLM using
            # FINAL(variable_name) instead of FINAL_VAR(variable_name))
            if environment is not None and _looks_like_variable(content):
                try:
                    # Use the same FINAL_VAR mechanism to resolve the variable
                    result = environment.execute_code(
                        f"print(FINAL_VAR({content!r}))"
                    )
                    resolved = result.stdout.strip()
                    # Only use resolved value if it's not an error and is longer
                    # than the variable name (indicates actual content was found)
                    if (
                        resolved
                        and not resolved.startswith("Error:")
                        and len(resolved) > len(content)
                    ):
                        return resolved
                except Exception:
                    pass  # Variable doesn't exist or other error, use literal content
