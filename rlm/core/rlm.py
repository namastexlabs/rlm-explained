...
class RLM:
    ...
    def completion(
        ...
        with self._spawn_completion_context(prompt) as (lm_handler, environment):
            ...
            for i in range(self.max_iterations):
                ...
                # Use streaming if logger supports it (for real-time token display)
                if self.logger and hasattr(self.logger, "log_token"):
                    iteration: RLMIteration = self._completion_turn_streaming(
    ...
    def _completion_turn_streaming(
        self,
        prompt: str | dict[str, Any],
        lm_handler: LMHandler,
        environment: BaseEnv,
        ...
        """
        Streaming version - shows tokens in real-time as they arrive.

        Streams tokens to the logger, detects code blocks as they form,
        and executes them immediately when complete.
        """
        iter_start = time.perf_counter()
        ...
        for token in lm_handler.stream_completion(prompt):
            full_response += token

            # Stream token to frontend
            if self.logger and hasattr(self.logger, "log_token"):
                self.logger.log_token(token, iteration_num)

            # Detect code block boundaries
            # Check if we just entered a code block
            if not in_code_block and "```repl" in full_response[max(0, len(full_response) - 20) :]:
                # Find the actual start of the code block
                idx = full_response.rfind("```repl")
                if idx >= 0:
                    in_code_block = True
                    code_start_pos = idx
            ...
            # Check if we just completed a code block
                ...
                # Look for closing ``` that's not the opening
                code_section = full_response[code_start_pos:]
                if code_section.count("```") >= 2:
                    # Extract and execute the code block
                    code = self._extract_code(code_section)
                    if code:
                        result: REPLResult = environment.execute_code(code)
                        code_blocks.append(CodeBlock(code=code, result=result))

                        # Stream code result immediately
                        if self.logger and hasattr(self.logger, "log_code_result"):
                            self.logger.log_code_result(iteration_num, code, result.to_dict())

                    in_code_block = False
                    code_start_pos = -1

        # Handle any remaining code block at the end
        if in_code_block and code_start_pos >= 0:
            code_section = full_response[code_start_pos:]
            if code_section.count("```") >= 2:
                code = self._extract_code(code_section)
                if code:
    ...
    def _extract_code(self, block: str) -> str:
    ...
    def _default_answer(self, message_history: list[dict[str, Any]], lm_handler: LMHandler) -> str:
        ...
        Explicitly asks the model for a FINAL() answer and parses it.
        """
        current_prompt = message_history + [
            {
                "role": "user",
                "content": (
                    ...
                    "Please provide your final answer now using the FINAL() syntax. "
                    "Example: FINAL(Your complete answer here)\n\n"
                    "Do not write any more code. Just summarize what you've learned and provide "
                    "the final answer in FINAL() format."
                ),
            }
        ]
        response = lm_handler.completion(current_prompt)

        # Parse FINAL() from response
        parsed_answer = find_final_answer(response)

        # If no FINAL(), clean up raw response (remove code blocks)
        if parsed_answer is None:
            final_answer = self._clean_fallback_response(response)
        else:
    ...
    def _clean_fallback_response(self, response: str) -> str:
        """
        Remove REPL code blocks from fallback response.
        Used when model doesn't provide FINAL() syntax.
        """
        ...
        return result or "Unable to generate a final answer within the iteration limit."
