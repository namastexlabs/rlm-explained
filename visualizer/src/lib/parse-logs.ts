import { RLMIteration, RLMLogFile, LogMetadata, RLMConfigMetadata, extractFinalAnswer, getCallTokens } from './types';
    ...
    for (const block of iter.code_blocks) {
      if (block.result) {
        ...
        if (block.result.rlm_calls) {
          ...
          // Aggregate token counts from all sub-LM calls
          for (const call of block.result.rlm_calls) {
            const tokens = getCallTokens(call);
            totalPromptTokens += tokens.input;
            totalCompletionTokens += tokens.output;
          }
        }
      }
    }
