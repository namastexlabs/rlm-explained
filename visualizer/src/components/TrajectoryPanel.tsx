...
function RoleIcon({ role }: { role: string }) {
  ...
  if (role === 'user') {
    return (
      <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-fuchsia-500 to-pink-600 flex items-center justify-center shadow-lg shadow-fuchsia-500/20">
        <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function RoleLabel({ role }: { role: string }) {
  const labels: Record<string, { name: string; color: string }> = {
    ...
    user: { name: 'User', color: 'text-fuchsia-600 dark:text-fuchsia-400' },
    assistant: { name: 'Assistant', color: 'text-cyan-600 dark:text-cyan-400' },
  };
        ...
        <ScrollArea className="h-full">
          <div className="p-4 space-y-4">
            ...
            {currentIteration?.prompt.map((msg, idx) => (
              <div 
                ...
                className={cn(
                  ...
                  msg.role === 'user' && 'bg-fuchsia-500/5 border-fuchsia-500/20 dark:bg-fuchsia-500/10',
                  msg.role === 'assistant' && 'bg-cyan-500/5 border-cyan-500/20 dark:bg-cyan-500/10'
                )}
              >
                {/* Message header */}
            ...
            {currentIteration?.response && (
              <div className="rounded-xl border-2 border-cyan-500/40 bg-gradient-to-br from-cyan-500/10 to-indigo-500/10 p-4 shadow-lg shadow-cyan-500/5">
                {/* Response header */}
                ...
                {/* Response content */}
                <div className="bg-background/80 rounded-lg p-3 border border-cyan-500/20">
