...
# API key environment variable names to check
API_KEY_VARS = [
    "OPENAI_API_KEY",
    "ANTHROPIC_API_KEY",
    "GEMINI_API_KEY",
...
def detect_configured_provider() -> tuple[str | None, list[str]]:
    """Detect if an API key is already configured.

    Returns:
        Tuple of (provider_name, list of configured key names)
    """
    ...
    # Map key names to provider names
    key_to_provider = {
    ...
    provider_name = key_to_provider.get(configured_keys[0], "Unknown")
    return provider_name, configured_keys
...
def main(force: bool = False):
    ...
    if not force:
        provider_name, configured_keys = detect_configured_provider()
        if provider_name:
            console.print(
                Panel(
                    f"[bold green]Already configured![/bold green]\n\n"
                    f"Provider: [bold]{provider_name}[/bold]\n"
                    f"Keys: {', '.join(configured_keys)}",
                    border_style="green",
                )
            )
            console.print()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="RLM Explained setup wizard")
    parser.add_argument(
        "--force",
        action="store_true",
